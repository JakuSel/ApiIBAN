    ...
    function IbanString(iban)
        {
            ...
            var plainIban = "SK"+checksumFinal+iban.code+"000000"+cisloUctu;
            return  plainIban.replace(/(\w{4})/g,"$1 ").replace(/(^\s+|\s+$)/," ").trim();
        }
...
if (BankaString(iban)!=null && iban.acc.length<11 && iban.code !=4 ){
    res.status(201).json({
